...
\begin{abstract}
%\boldmath
Engineering software is a social activity. Software projects' success is very often dependent on how the welfare of their development community. This connection becomes especially delicate when multiple communities have to join forces (e.g. in global software engineering). So far however, software engineering discipline lacks systematic methods to steer development communities. In this paper we present a method to snapshot development communities. We elaborated the method conducting a real-life case-study in a large software development organisation. 
One key finding is that the method can be generalised. In addition, from the case-study we found that community snapshots can have a limited number of forms, each we found many organisational barriers, both negative and positive ones. 
...
The study resulted in a semi-automatic method to ``snapshot'' a development community. A community \emph{snapshot} is obtained by combining two key pieces ofinformation: (a) answers to a questionnaire (see Fig. \ref{quest}); (b) visits to a decision-tree (presented in previous work \cite{specissue}). 
