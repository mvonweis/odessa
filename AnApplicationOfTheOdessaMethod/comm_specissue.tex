...
\title{``Snapshooting'' Development Communities}
...
For example, imagine a project manager wants to find out how does his development community looks like, after his development company extends it with offshore partners and open-source communities. To ``run'' ODeSSA the project manager can feed the ODeSSA Questionnaire (e.g.~through an online surveying tool). Once data is obtained, the project manager can elaborate the \emph{snapshot} represented in the questionnaire mapping the answers on the decision-tree. Finally, with the resulting \emph{snapshot}, the project manager can understand and further analyse the current status of his development community using the framework discussed in this paper. Figure \ref{snapshot} outlines our idea of a very simple \emph{snapshot}. The figure represents what results if ODeSSA is applied to analyse text-book project teams in closed and clear-cut organisational boundaries.
...
ALSO DISCUSS ABOUT SENSE-MAKING AND THE CYNEFIN METHOD\\
DISCUSS HOW WE ANALYSED THE DATA AND EVIDENCE\\
...
REVISE\\
INTEGRATE\\
EXTEND WITH A COMPLETE SNAPSHOT FOR THE CASE STUDY: INCLUDING WGs AND NOPs\\
DESCRIBE SITUATION AT COMPANY X AND HOW THE SNAPSHOT IS CONSISTENT WITH REALITY AND EMPIRICAL EVIDENCEÉ MARTIN?\\
