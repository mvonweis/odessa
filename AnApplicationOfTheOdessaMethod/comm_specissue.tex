...
% MARTIN COMMENT: The following couple paragraphs contain redundancies. 
...
The method stems from previous work. In \cite{specissue} we introduced a decision-tree to uncover social communities observable in an organisation, e.g. a global software project. The decision-tree uses a partial-order relation among social communities to pinpoint the community type emerging in the observed case.

The issue we tackle in this paper is how to map an actual software development organization onto this decision tree. We do so through a case study at a large industrial organization. The mapping turns out to be non-trivial, and so is the process for doing so. This paper elaborates on the case study and the SEEDS process that emerged.
...
The mapping results in a ``colouring'' \cite{networks} of the decision-tree from \cite{specissue}. This ``colouring'' is a  \emph{snapshot}, i.e. an instantaneous configuration of the observed community. 
