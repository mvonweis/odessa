...
\hline 
\end{tabular}
\caption{ODeSSA Questionnaire.}\label{question}
\end{table}
...
\subsection{ODeSSA: The Questionnaire}\label{quest}
...
\subsection{ODeSSA: The Checklist}
...
%\end{wrapfigure}
\end{figure}
\subsection{ODeSSA: The Decision-Tree}\label{dectree}
...
\subsection{ODeSSA: An Analysis Framework}\label{fw}

Inheriting from strategic SWOT evaluation approaches such as \cite{swot}, we elaborated an initial analysis framework for ODeSSA \emph{snapshots}. To understand strengths, weaknesses, opportunities and threats using ODeSSA \emph{snapshots}, there are two key analyses that can be performed. Figure \ref{fwpic} shows the resulting evaluations that can be performed (on the left-hand side) using either decision-tree forms or desirable properties (right-hand side).

First, observers must evaluate the ``form'' of the snapshot. ODeSSA \emph{\emph{snapshot}s} can assume three possible forms: 
\begin{enumerate}
...
\item \emph{enriched-community}: the answers in the questionnaire reflect one (or more) path(s) blended with additional ``enriching'' attributes (including pivots) but no additional paths. This form evidences a \textbf{threat} for the community.
The enrichments can indicate reasons for failing organisational performance. Practitioners can use the evidence to fill-in with missing support. For example, suppose the ``visibility'' attribute is discovered in mix with the Formal Networks community type. Formality within formal networks might have forced the organisation to adopt mechanisms to increase the visibility of projects, people or artefacts, perhaps it is wise to foster the generation of a Knowledge Community jointly with the Formal Network. Refinement and explicit support to such technologies includes the adoption of Information Management or Social Networking technologies \cite{eis,eim} to support development community.
