...
Second, ICs were found in sites collaborating strongly with open-source. From \cite{ossslr} we know that ENGAGEMENT is the key-defining attribute of ICs (see Section \ref{rel}). Management at Company X must explicitly support ENGAGEMENT in sites where ICs were identified.
...
The three areas or ``foci'' above, are higher-order properties of a community. 

The community at Company X did not seem to require any additional focus.
...
Fifth, finally, there are many dimensions to be found using SEEDS. While formulating decision-tree foci, we assumed that organisations would globally focus on communities that fostered ``networking'' (right-hand side of the decision-tree). Conversely we assumed that organisations would concentrate on certain ``goals'' (left-hand side of the decision-tree) locally in individual sites. We found that Company X exhibits an opposite behaviour. Company X focuses globally on a particular goal, i.e. maintaining aligned the organisational structure on a particular goal. Moreover, Company X focuses locally on networking and informally collaborating with open-source. \emph{Our data was insufficient to make any conclusive statement. However, we noticed that SEEDS and the decision-tree have much more to tell us about understanding and interpreting communities and we have barely scratched the surface.}
...
The three SEEDS \emph{snapshot} forms described in Section~\ref{fw} can also be explained in terms of the Stacey matrix \cite{sta02aa}. The matrix was introduced by Ralph Stacey to help make sense of management approaches using the two axes of ``agreement'' and ``certainty'' as depicted in Figure~\ref{fig:stacey}.
