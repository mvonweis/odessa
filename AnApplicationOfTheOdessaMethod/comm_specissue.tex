...
{\bf Objective:} This paper presents a method to understand a development community by ``\emph{snapshoot}ing'' its configuration with observable key characteristics in an intelligible form. The method aids, for example, diagnosis of socio-technical problems in the community: (a) identifying unwanted circumstances within community that can be causing trouble; (b) elaborating strategies to change or steer a community, using key characteristics that describe its (current) status; (c) identify conflicting properties within the community.
...
This reasoning led us to switching our course of action: instead of visiting the decision-tree, incrementally looking for data to answer each node, whilst preparing for more visits, we prepared a list of questions representing nodes in decision-tree. This move made our approach more focused. After analysing available data twice, we were able to exactly pinpoint areas of text explicitly and unmistakably answering questions.
...
\subsection{Habemus ConfigurationÉ So What?}
...
We analysed our results and made five key observations. The following text states the observations and provides an interpretation in \emph{italic}.
