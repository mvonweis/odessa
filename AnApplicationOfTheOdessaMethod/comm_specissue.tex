...
\title{``Snapshooting'' for Diagnosing Development Communities}
...
{\bf Objective:} This paper presents a method to diagnose a development community by ``\emph{snapshoot}ing'' a community by capturing its observable key characteristics in an intelligible form. The method aids diagnosing socio-technical problems of development communities. For example, the method helps: (a) identifying unwanted circumstances within community that can be causing trouble; (b) elaborating strategies to change or steer a community, using key characteristics that describe its (current) status; (c) identify conflicting properties within the community.
...
The method uses a questionnaire/checklist to gather key information needed. Subsequently, the method uses a checklist and decision-tree from \cite{specissue} to outline the development community. The decision-tree is the result of a partial-order relation among key attributes of communities. Through this partial relation it's possible to distinguish one (or more) community from others. The configuration, or \emph{snapshot}, of an observed community is obtained as a combination of key community properties \cite{ossslr}. This combination of properties is obtained answering a questionnaire designed to identify the presence of said key properties. Answers can be found flexibly, from many sources of information, for example: (a) the questionnaire can be sent to developers; (b) observers of a community can analyse data describing organisational structure and processes of the community; (c) social-network analysis. Once the questionnaire is filled-out, answers need to be mapped onto the decision-tree to determine the community types present in the observed community. The mapping results in a ``colouring'' \cite{networks} of the decision-tree from \cite{specissue}. This ``colouring'' is a  \emph{snapshot}, i.e. an instantaneous configuration of the observed community, obtained through known key properties of communities. 
...
This section outlines previous work building the foundation of the method presented in this paper. In addition, we investigate on relevant research efforts related to our own.
%\begin{itemize}
...
One of the agile coaches in the case organisation was able to give continuous feedback throughout the research and the writing of this report. This person clarified the structures and working modes inside Company X and provided deeper insight into one of the departments (the same covered by Ref.~2). 
...
To pursue the above goal, we started applying previous work from \cite{specissue} to our large industrial case-study. The work in \cite{specissue} presents a decision-tree that allows to identify community types (summarised on Table \ref{commtypes}) by means of key defining attributes. Our initial approach was try to colour the decision-tree from \cite{specissue} with available data. We were immediately presented with problems that we solved incrementally.
...
We used a parallel case-study, conducted by our industrial stakeholder, (referred to as \emph{Sub-case 2}, from now on) to confirm the validity of SEEDS. We structured the \emph{Sub-case 2} parallel study with observer and data triangulation to minimise bias and unreliability following guidelines from \cite{stake}. The parallel case-study was conducted by an insider of Company X, using data available as well as his own expertise and experience/knowledge about Company X. While the two studies were started at the same time, the researcher conducting the first study (called \emph{Sub-case 1}) was prohibited to share increments of partial results. Weekly meetings were planned to assess progress status only. The supervision of two senior researchers was used throughout the duration of the study to assess the quality of progress.

All along the (re-)application of SEEDS, data sources triangulation \cite{stake} and observer triangulation were used to tackle possible threats to validity, as discussed in Section \ref{disc}. 
...
Answers to the SEEDS questionnaire are summarised on Table \ref{questionnaire1} (for Sub-case 1), and on Table~\ref{questionnaire2} (for Sub-case 2). Column 1 reports the question needed to gather necessary information. Column 2 and 3 present answers that we found in our two parallel case-studies.
...
\hline 
\textbf{\emph{SEEDS Questions}} & \textbf{\emph{Sub-case 1 Answers}}\tabularnewline
\hline 
\hline 
...
%\hline 
%\end{tabular}
\caption{SEEDS case-study, answers to the questionnaire for Sub-case 1.}\label{questionnaire1}
\end{table}
...
\begin{tabular}{|>{\raggedright}p{4cm}|>{\raggedright}p{8cm}|}
\hline 
\textbf{\emph{SEEDS Questions}} & \textbf{\emph{Sub-case 2 Answers}}\tabularnewline
\hline 
\hline 
\emph{\footnotesize Can you distinguish an organised structure across
...
\hline 
\end{tabular}
\caption{SEEDS case-study, answers to the questionnaire for Sub-case 2.}\label{questionnaire2}
\end{table}
...
A deeper analysis of the checklist in Fig.~\ref{list} comparing with data from Tables \ref{questionnaire1} and \ref{questionnaire2} reveals two \emph{pivots} for Company X. Pivots reside, namely, in the DISPERSION and FORMALITY nodes. 
...
\subsection{SEEDS Limitations and Threats to Validity}
%%%%\begin{itemize}
