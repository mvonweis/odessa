...
\title{Taking Snapshots of Development Communities}
...
%{\bf Conclusion:} We conclude that SEEDS efficiently determines \emph{snapshots} for software development organizations. \emph{snapshots} can be used to govern a software development community.\\
...
{\bf Objective:} This paper presents a method to understand a development community by ``\emph{snapshooting}'' its configuration with observable key characteristics in an intelligible form. The method aids, for example, diagnosis of socio-technical problems in the community or elaborating strategies to change or steer a community, using key characteristics that describe its status.

{\bf Method:} To pursue our objective, we developed SEEDS, a method for ``\emph{Snapshots}-based Evaluation and Evolution of Development communitieS''. 

{\bf Results:} In validating SEEDS through a case-study, we found the method allows reliable information about the strengths, weaknesses and inconsistencies of a development community. 

{\bf Conclusion:} We conclude that \emph{snapshots} for software development communities, as determined by SEEDS, help understanding development communities.
%
%
...
Software Engineering; Human Factors; Social Structures; Software Development Communities; Networked Organisations; Community Performance; Community Complexity; Socio-Technical Decisions; Socio-Technical problems
\end{keyword}
...
%The craft of software development is becoming more and more a social activity \cite{specissue}. Several studies suggest that engineers spend between 70 and 85\% of their time working with other people (end-user focus groups, managers, business sponsors etc.) \cite{socialbook}. This means that in \mbox{70--85\%} of the cases, software engineering success is a community effort rather than a success of individuals.
%This figure becomes critical with the increase of diversity and size of the development community.
%For example, Global Software Engineering (GSE) is a development strategy entailing global teams to collaborate from different locations in different timezones \cite{gsdbook}. In this circumstance, distances in time, space and culture magnify project risks connected to attributes of the development social structure \cite{empirglob,nachiappan,icgseoss}.
...
The craft of software development has always been considered a social activity \cite{naur1969,wei98aa,bro95aa}. Several studies suggest that engineers spend between 70 and 85\% of their time working with other people (end-user focus groups, managers, business sponsors etc.) \cite{socialbook}.
%This means that software engineering is to \mbox{70--85\%} a community effort, rather than individual work.

This figure becomes critical with the increase of diversity and size of the development community.
For example, Global Software Engineering (GSE) is a development strategy entailing global teams to collaborate from different locations in different timezones \cite{gsdbook}. In this circumstance, distances in time, space and culture magnify project risks connected to attributes of the development social structure \cite{empirglob,nachiappan,icgseoss}.
The problem persists since software practitioners lack a systematic method to outline the organisational state of development communities and use this ``picture'' to understand them.
% MARTIN COMMENT: Ref needed above!

To tackle this problem, this paper proposes SEEDS, a semi-automatic method for ``Snapshot-based Evaluation and Evolution of Development communitieS''.
The purpose of the SEEDS method is to capture a ``\emph{snapshot}'' of a development community so that the configuration of the community can be plotted and its key properties captured, analysed and reasoned on.

The SEEDS method stems partially from previous work \cite{specissue}, in which we made a literature review to identify a number of social community types and their attributes. We found that each community type has a unique \emph{key-defining} attribute and that the partial-order relation between community-attribute pairs form a decision-tree. This decision-tree can be used to pinpoint the relevant community type of an observed case organisation, e.g.~a global software project.

To validate the framework we mapped an actual software development organisation onto the decision tree. The mapping itself and the mapping process turned out to be non-trivial, and the case study and the SEEDS process that emerged now forms the basis of this paper.
...
%
%Putting together the sequence of steps employed during our case-study, we were able to work out a general method to plot the configuration of a community. 
%
%The method flexibly uses multiple inputs, for example: (a) the SEEDS questionnaire can be sent to developers; (b) observers of a community can analyse data describing organisational structure and processes of the community; (c) social-network analysis. 

%Once the questionnaire is filled-out, answers need to be mapped onto the decision-tree from \cite{specissue} to determine the community types present in the observed community. 

The mapping results in a ``colouring'' \cite{networks} of the decision-tree from \cite{specissue}. This ``colouring'' is a  \emph{snapshot}, i.e.~an instantaneous configuration of the observed community. Figure \ref{snapshot} outlines an example of such a \emph{snapshot}, the result of applying SEEDS to analyse text-book \cite{hansbook} project teams in closed organisational boundaries. The resulting \emph{snapshot} exhibits a complete path (see Fig.~\ref{snapshot}, path from root to the PT rounded node for Project Teams) and no additional properties on the SEEDS decision-tree (all edges remaining coloured are ``NO'' answers to the decision-tree).
...
To get an understanding of SEEDS in practice, imagine that a manager wants to find out what his development community looks like, after his development company extends it with offshore partners and open-source communities. To ``run'' SEEDS the project manager needs only use four tools, in sequence: (1) a questionnaire; (2) a checklist; (3) a decision-tree; (4) and finally, a simple evaluation framework.

First, management can feed the SEEDS questionnaire (e.g.~through an online surveying tool) to all sites involved. Second, with data from the questionnaire, management can use the SEEDS checklist to structure the data (e.g.~to reveal multiple-answer points). Third, management can understand community types mapping the checklist on the SEEDS decision-tree, obtaining a \emph{snapshot} of the observed community. Fourth, management can apply the SEEDS framework on the resulting \emph{snapshot} to analyse the current status of the observed development community. 
% TODO ELABORATE MORE ON THE PART ABOVE HERE AND MAP COMPLETELY WITH THE STEPS IN SECTION OF SEEDS

The benefits of our method are manifold: it is (a) \emph{flexible} - SEEDS is based on key community characteristics that can be discovered in multiple ways, allowing flexibility to industrial practitioners with diverse information and time available; (b) \emph{straightforward} - SEEDS is based on filling a questionnaire and colour a decision-tree, it can be used by practitioners coming from very different backgrounds; (c) \emph{multi-purpose} - SEEDS can be used to understand development communities from multiple angles, e.g.~to study the current situation for change-planning or improve development performance by removing communication barriers. 
...
\end{figure}
Applying SEEDS, we made three key observations: (a) the method allows uncovering inconsistencies on the observed \emph{snapshot}; (b) the community \emph{snapshot} can have a limited number of possible forms, each with its own particular characteristics; (c) the method allows uncovering properties that can hinder communication/collaboration across communities during software development. 
%
%Building on previously published work, our paper offers the following novel contributions: (a) the SEEDS method, validated through a large industrial case-study; (b) a questionnaire and checklist that feature feedback from industrial practitioners; (c) analysis guidelines that allow interpretation of the \emph{snapshots} found. These guidelines were obtained applying SEEDS in practice and feature feedback from industrial practitioners. 

The rest of the paper is structured as follows: Section \ref{devcomm} explains related work; Section \ref{mm} explains materials and methods; Section \ref{cs} provides the industrial case-study carried out to determine SEEDS, while Section \ref{method} presents SEEDS. Section \ref{disc} discusses both SEEDS and its application. Finally, Section \ref{conc} concludes the paper with suggestions for future work.
...
This section discusses previous work in building the foundations of the method presented in this paper.  This discussion is necessary to understand SEEDS and its theoretical underpinnings. Finally, we  discuss relevant research efforts related to our own.
%\begin{itemize}
...
% MARTIN COMMENT: I find that the following paragraphs (down to the list of decision nodes) could use a rewrite, as there's some repetition and lack of clarity. This is not a critical problem though. We also need to balance the amount of detail given here against the new work in sections 4-5.

In previous work we found that observable \emph{social communities}, e.g.~as captured in \cite{Davenport2001} are a composition of known social community archetypes \cite{ossslr}. Each community type can be given a distinct characterisation through key observable properties.
The combination of these properties can be used to ``plot'' a community configuration (by representing together its ``current'' properties and attribute values) as well as study its performance (by comparing over time different ``configurations'').
In \cite{ossslr} we presented an overview of the state of the art in social communities, offering a distinct characterisation for each type. The state of the art is summarized in Table \ref{commtypes}. The table presents name and description of community archetypes from \cite{ossslr}. In the description, we also discuss the key defining attribute for each type.
The work presented in this paper goes one step further, by introducing a method to ``\emph{snapshot}'' software development communities using key-defining attributes. The method draws motivation and foundations from the work in \cite{icgseoss,ossslr}.
...
Then we divided common attributes of each type into two groups: (a) common attributes that had only one value, and (b) common attributes that had a number of different values. 
We found that, for most types, group (b) counted many elements and many values. Conversely, group (a) were populated by a single element for each type. 
From this, we made two key observations. 
First, the singletons in groups (a) uniquely identify a type.
Second, combining groups (a) and (b) together characterises the community type. We named the singletons in group (a) as \emph{key-defining} attributes.

% MARTIN COMMENT: Perhaps an pair of examples could be useful here? For example "visibility" and Knowledge Community?

%Also, the method was inferred by trying to validate the decision-tree from \cite{specissue}.  
%The notion of \emph{key-defining} attribute is not applicable to all cases. What happens if we observe multiple \emph{key-defining} attributes together in a real-life situation? We must then be able to relate this observation to the corresponding social community types. 


Analysing the \emph{key-defining} attributes we found many relations between them: for example, some implied or mutually-excluded other. The relations formed a partial-order function for \emph{key-defining} attributes. A partial-order function associates an ordering or sequencing to the elements of a set. A real-life example of a partially-ordered set is a family genealogical tree, where some pairs of people bear the descendant-ancestor relationship, but not all pairs.

% MARTIN COMMENT: The following paragraphs are only fragments.
%
%The decision-tree stems from the need to differentiate between community types in the (realistic) case more key-defining attributes are observed together.
...
The decision-tree is now an essential part of SEEDS.

The nodes on the tree are labelled with \emph{key-defining} attributes (e.g.~if a ``STRUCTURE'' and ``DISPERSION'' are observable then type Networks of Practice (NoP) is identified, i.e.~we answer YES to just those attributes in the path from root to leaf nodes). When visiting the tree, the observer needs to decide wether the key defining attributes are present or not. Each decision can be formulated in terms of a question (e.g.~``Do community members use informal communication only?''). These questions can be easily answered by observing any organisation, be it a single development effort, or an entire enterprise. 
...
\hline 
\end{tabular}
\caption{Social community types, summary from \cite{ossslr,specissue}.}\label{commtypes}
\end{table}

\subsection{Related work: Organizational Structures and their Impact on Software Quality}

Literature in GSE already recognises the need to study global communities. In particular, many works such as \cite{nachiappan,nid95aa} have studied empirically the effect of organisational structures on quality and success in software projects. These works motivate further studies to understand, represent and support social communities. In addition, similar works \cite{uls,rel9} study the effect of socio-technical congruence on designing global systems as well as the process of global engineering.

Clearly expressed in this line of research is that understanding how coordination requirements map to their human counterpart is critical to successfully steer information across global communities. Our work is limited to identifying community types, and finding ways to ``format'' and study their observable characteristics. 
...
Finally, works such as \cite{rel,rel4} investigate the impact of organizational decision-making on software development. For example, understanding if the current community \emph{snapshot} is performant (or even compatible) with offshoring is essential for the community to ``go global'' \cite{icsesympo}. The method presented in this paper can be used to support organisational decisions, investigating the current and to-be community \emph{snapshots}. SEEDS enables understanding the consequences of change to the development community. For example the adoption of agile methods can cause distress within the community and incite the emergence of organisational barriers \cite{Correia2010}.
...
The evidence used in our work stems from empirical research conducted in a big organisation active in the mobile technologies market.
...
The organisation (called Company X from now on) is a multinational that develops both hardware and software components for end-users' mobile-phones and apps to enrich user experience. Company X employs over 100~000 people with sales in over 160 countries. At the time our study was conducted (early 2012), the organisation had a net market sale of over 40 billion euros.
...
Three key sources of evidence were used, as described below. We use page numbers to trace answers to the SEEDS questionnaire back to the data.

\begin{enumerate}
\item The first key resource (called \emph{\bf Reference~1}) is a 112-page research report on agile and open-source adoption in Company X. The study was conducted using grounded-theory \cite{gt}. The report features six interviews with focus groups from Company X. Focus groups had varied expertise, from areas such as project management to agile coaches to code developers. The focus sessions are analysed through grounded theory. The study offers an overview of the organisational structure of Company X as a whole (i.e.~as a global community) as well as explaining its local sites. The research focuses on a division of Company X, working on a specific project release at the time of investigation. 

\item The second key resource (called \emph{\bf Reference~2}) is a 97-page research report on three software engineering teams within Company X. The report features 16 interviews harvested through random and snowball sampling \cite{Goodman}. The sample consisted of both sex, however, with a minority of females. Interviewees had varied backgrounds but an average work experience in Company X of about 10 years. As a result, the organisational details of this paper focus on software process, software coding methods, development best-practices adopted, problems found, and ultimately, the people's perspective on all the above.

\item The third key resource (called \emph{\bf Reference 3}) is a four-page subset of the tutorial presentation that agile coaches within Company X used to introduce X's organisational structure/process to newly appointed staff members. The slides provide a visually oriented overview of the agile process and open-source integration protocols in place within Company X at the time of investigation.
\end{enumerate}

Additionally, one of the organisational coaches (called the \emph{\bf Industrial Partner}) in the case organisation was able to give continuous feedback throughout the research and the writing of this report. The Industrial Partner clarified the structures and working modes inside Company X and provided deeper insight into one of the departments. The Industrial Partner had twenty years of experience in the ICT industry and five years of experience in Company X.

All of the written sources mentioned above are dated around 2012 and describe the company as it was in 2010 or 2011. These sources are protected by non-disclosure agreements\footnote{Available upon signed request.}. At the time of investigation, Company X was going through many organisational changes and both studies aimed at understanding the impact of organisational change on teams, their production velocity, community productivity and end-product quality.


\subsection{Research Methods}
%TODO:\\
%ALSO SPECIFY HOW WE ARRIVED TO THE SEEDS METHOD\\
%BTW HOW DID WE ARRIVE TO THE ODESSA METHOD?\\
...
To pursue the above goal, we started applying previous work from \cite{specissue} to our large industrial case-study, following guidelines in \cite{runeson}. 
%The work in \cite{specissue} presents a decision-tree that allows to identify community types (summarised on Table \ref{commtypes}) by means of key defining attributes. Our initial approach was try to colour the decision-tree from \cite{specissue} with available data. We were immediately presented with problems that we solved incrementally. 
%
...
To validate in practice the decision-tree introduced in \cite{specissue}, we analysed the evidence introduced in Section~\ref{mm}. We needed data to determine if certain key attributes of social communities were present in the case at Company X. Based on \cite{specissue} and \cite{ossslr}, all the nodes in the decision-tree from \cite{specissue} (see Section \ref{nodes}) needed answering.
...
%DESCRIBE SITUATION AT COMPANY X AND HOW THE \emph{snapshot} IS CONSISTENT WITH REALITY AND EMPIRICAL EVIDENCEÉ MARTIN?\\
%
...
The vivid scenario above reflects a large quantity of data from our resources. 
%Given this sheer amount of data to answer nodes in the decision-tree we noticed that many visits to the decision-tree were needed to exhaust all the information available. This would have introduced a fundamental flaw in our validation attempt, since we would be applying a partially validated framework over and over again.
In order to interpret this data, we initially tried to visit the decision-tree in a top-down fashion. Given the amount of data and the fact that it was collected for other purposes than organisational analysis, we quickly found that this approach was not working.

This led us to switching our course of action. Rather than visiting the decision-tree top-down, looking for data to answer each node, we instead prepared a list of questions representing nodes in the decision-tree. This allowed us to ``visit'' all nodes in parallel, delaying the drawing of the decision-tree until all data was properly organised in a questionnaire. Analysing the available data, we were able to exactly pinpoint the areas of text that explicitly and unmistakably answered the questions.
% SAY EXPLICITLY THAT WE TRIED TO VISIT THE TREE TOP DOWN BUT THAT WAS NOT WORKING AND WE HAD TO VISIT ALL NODES AT THE SAME TIME IN A SENSE
% MARTIN COMMENT: What does "analysing the available data twice" actually mean?

Answers to this questionnaire are summarised in Table \ref{questionnaire1} for Study~1, and in Table~\ref{questionnaire2} for Study~2. Column~1 reports the questions needed to gather necessary information. Column~2 presents answers that we found in our two studies.
...
yourself and your colleagues?} & {\footnotesize Evidence available in \emph{\bf Reference 3} points out to a non-trivial
...
\emph{\bf Reference 2} + \emph{\bf Reference 3}).}\tabularnewline
\hline 
\emph{\footnotesize Do you work on time-bound projects?} & {\footnotesize In page 2 of \emph{\bf Reference 3} the strategic portfolio duration
...
\hline 
\emph{\footnotesize Do you and your colleagues disseminate or increase
the visibility of your work? } & {\footnotesize The teams formed their own information
silos regarding project and product metadata. While the organization required some data sharing for portfolio management purposes, the teams were reactive
rather than proactive in dissipating information. }\tabularnewline
\hline 
\emph{\footnotesize Do you and your colleagues actively try to team-build? } & {\footnotesize The organization didn't explicitly
promote cohesion. Few team-building exercises were proposed or promoted. However, the Scrum working
model proposes reasoning and reflection sessions that do increase cohesion. }\tabularnewline
\hline 
\emph{\footnotesize Do you need to explicitly demonstrate ROI to your
...
of the interviews: \textquotedblleft{}It\textquoteright{}s fantastic
that I can work on my open source hobby project all day, and get paid for it!\textquotedblright{} }\tabularnewline
\hline 
\emph{\footnotesize Do you and your colleagues actively gather and
...
%%% CAUSED BY MORE THAN ONE ANSWER WE NEEDED TO STRUCTURE THE DATA, EXPLAIN PIVOTS IN A BRIEF SHORT SENTENCE

After answering questions to gather data about communities at Company X, we felt that something was missing. In particular, we missed a way to express variable answers to questions in the questionnaire. While most data confirmed many answers over and over again, some data revealed conflicts or multiple possible answers to some questions. For example, our case revealed two points with multiple answers, namely, in the DISPERSION and FORMALITY nodes. 
We found that these points deserved further investigation. 

Brainstorming with the Industrial Partner and digging deeper in the data, we found that the first point, DISPERSION, identified a sub-community. The second point, FORMALITY, identified an inconsistency. Both these points were variation points within the observed community. Based on this nature, we named them \emph{pivots}.

\emph{Pivots} however, were not explicit in our questionnaire. Rather they resided in the multiple versions of the same questionnaire, e.g.~answered with different sets of data. To make \emph{pivots} explicit we decided to use a Checklist.

The checklist in Fig.~\ref{list} contains answers from both our parallel studies. Answers are marked with the label ``[\emph{x}]'' where \emph{x} is either of our two \emph{studies}. The checklist also makes it possible to spot attributes for which we did not have enough data to answer. For example, in our case, none of our data sources contained evidence to clearly describe Company X' s FORMALITY practices as present or absent. Our Industrial Partner however, was able to fill-in for \emph{Study 2}, using own data and experience within Company X. 
%To emphasise the multiple nature of these key-points in our ``picture'' we used the term  , since they are points of indecision that distinctively characterise the configuration for Company X.

The first \emph{pivot}, DISPERSION, is caused by the presence of two different types of data. Each type led to a different answer. The first type refers to Company X as a whole, describing the overall structure and operations. The second set refers to specific sites within Company X, describing their own operations, and their collaboration with open-source communities. Consequently, the specific sites work in a way different than Company X as a whole.

The second \emph{pivot}, FORMALITY, is caused by an inconsistency within the observed community. While the Industrial Partner gave a straight YES answer to the FORMALITY question, parts of the data at our disposal led us to find that Company X did not adopt formal evaluation of members. Other parts led us to find that Company X did use formal procedures and background checks to enrol members in teams or other project development units. When presented with the snapshot, the Industrial Partner identified the inconsistency. One root cause of the inconsistency was the collaboration between direct employees and developers from the open source communities. While people directly appointed by Company X were formally evaluated, people from open-source rarely went through any background or experience checks.

%\emph{Pivots} however, were not explicit in our questionnaire. Rather they resided in the multiple versions of the same questionnaire, answered with different sets of data. 
%Data from Tables \ref{questionnaire1} and \ref{questionnaire2} is consistent with the checklist in Fig.. 
...
Structured with the checklist in Fig. \ref{list}, ``colouring'' the decision-tree from \cite{specissue} is effortless. Figure \ref{result} shows the colouring. The colouring evidences the two pivot points, DISPERSION and FORMALITY as well as three social communities, a plain Social Network (SN), Formal Networks (FNs) and Informal Communities (ICs). Pivot points are shown in ovals, while identified social community types are shown in circles.
...
The first pivot point in the DISPERSION node identifies a sub-community. The colouring reveals the sub-community to be Informal Communities (ICs), emerging in individual sites. Enthusiastic engagement and collaboration both from open- and closed-source contributors are key characteristics. Frequent and formal collocated meetings are used to verify productivity. 

The second pivot point in the FORMALITY node identifies an inconsistency. Some sites worked as FNs, adhering to strict and closed governance guidelines, precluding formal evaluation of people and their results. This was inconsistent with management's expectations for informality, given the potential collaboration with open-source communities. 
% ASK MARTIN TO EXPLAIN THE INCONSISTENCY BETTER

Our data also leads to the presence of additional attributes for Company X, namely GOVERNANCE practices and DURATION. These attributes elaborate further on the observed configuration for Company X.  
% EXPLAIN WHY THIS IS ENRICHING

%%%%% EXPLAIN WHY ONE IS ENRICHING AND THE FNs ONE IS AN INCONSISTENCY
...
The above description, represents the instantaneous configuration of the organisation at Company X, plotted using key attributes from literature. This configuration is a \emph{snapshot}.
...
Across many brainstorming sessions we found ways to investigate the \emph{snapshot} to really understand the community configuration it represented. 

%\begin{center}
\subsubsection{Concerning Community Complexity}
%\end{center}
...
%\begin{center}
\subsubsection{Concerning Attributes}
%\end{center}
%%%%%%%% ENRICH THE DISCUSSION WITH DETAILS AND WHY AND DESCRIBE THAT IT COMES FROM THE SWOT MODE - WHICH YES ANSWERS DO YOU EXPECT?? YOU HAVE YESSES WHERE YOU EXPECT AND YOU HAVE YESSES WHERE YOU DONT

The complex form at Company X is reflected in three community types (SNs, ICs and FNs), two pivots and additional attributes. To analyse this form we searched for three things: (a) the key-defining attributes of communities found; (b) attributes that were desirable but were not found; (c) conversely, attributes that were undesirable but were indeed found. 

First, SNs were found globally for Company X. The goal for the SN was maintaining (updated) social links across Company X. With the \emph{snapshot} at hand, management can now decide better how to support this goal.

Second, ICs were found in sites collaborating strongly with open-source. From \cite{ossslr} we know that ENGAGEMENT is the key-defining attribute of ICs (see Section \ref{devcomm}). Management at Company X must explicitly support ENGAGEMENT in sites where ICs were identified.

Third, FNs were found in sites not currently collaborating with open-source. This contrasted with corporate policies of strong collaboration with open-source. FORMALITY in this case is an undesirable attribute. Rather than fostering FORMALITY, management must lift some formal evaluation/governance procedures to allow a more informal community to thrive.
%
%More than trivially showing that Company X was a complex organisation, we decided to use our \emph{snapshot} to look for desirable properties. This meant posing the questions:  or conversely ``was there anything you did not find but would have liked?''.

Finally, found that the \emph{snapshot} evidenced the VISIBILITY attribute set to NO. Conversely, empirical evidence from \textbf{Reference 1} and \textbf{2} indicated that members of Company X, from multiple sites, identified the need for increased VISIBILITY of practice, artefacts and people. For example, one of the interviewees part of \textbf{Reference 1}, responded with the following quote: ``all teamsÕ backlogs are [or must be] transparent and product management really needs to prioritise the content (features) of those backlogs''. Management at Company X can now plan for practices to explicitly support VISIBILITY across their sites.

%\begin{center}
\subsubsection{Concerning Steering Directions}
%\end{center}

Finally, inspecting the decision-tree, we also found that it can be divided in three higher-order sets of attributes. 
...
\item \emph{Goals:} the NO-branch out of the DISPERSION node, identifies community archetypes whose purpose is to pursue a particular goal, under certain conditions. The community's focus is its goal (e.g.~an organisational problem or Return-on-Investments).
\end{enumerate}

The three areas or ``foci'' above, are higher-order properties of a community. These foci can be seen as steering directions for the observed community. If the observed community shows evidence of a missing focus, then management can use the snapshot to drive the community in a specific direction (e.g.~specifying explicit organisational problems for business units).

However, we found the community at Company X did not seem to require any additional focus.
%We analysed \emph{Snapshots} to identify the missing focus, i.e.~a \emph{Higher-order need}. However, we found the complex community at Company X does not seem to require additional steering to include missing foci.

\subsection{Evaluating the status of Company X}

In summary, the \emph{snapshot} allows associating Company X with the following status:

\begin{enumerate}
\item \emph{\bf Three social community types are present: SNs, ICs and FNs}. While Company X looks like a SN as a whole, some sites (specifically, sites collaborating with open-source communities) work as ICs. Compared to IC sites, other sites work as FNs, this can cause inconsistencies or lack of communication/collaboration.
...
This section discusses our results and key observations. The section starts by discussing general observations on our case-study and SEEDS. Later, section \ref{sec:complexity} discusses how SEEDS can be used as a framework to establish or triangulate the  community complexity. Finally, section \ref{ttv} discusses threats to validity of this study, including SEEDS' limitations.


%%% WHEN FIRST WE NOITCUED THAT A GLOBAL OGANIZTION HAS A TYPE ON THE LEFT HAND SIDE, EHRE FOR NOKIA IS THE OTHER WAY AROUND
...
%
%{\bf First, the questionnaire part of the SEEDS methodÉ }
...
Fifth, finally, there are many dimensions to be found using SEEDS. For example, while formulating decision-tree foci, we assumed that organisations would globally focus on communities that fostered ``networking'' (right-hand side of the decision-tree). Conversely we assumed that organisations would concentrate on certain ``goals'' (left-hand side of the decision-tree) locally in individual sites. We found that Company X exhibits an opposite behaviour. Company X focuses globally on a particular goal, i.e.~maintaining aligned the organisational structure on its mission. Moreover, Company X focuses locally on networking and informally collaborating with open-source. \emph{Our data was insufficient to make any conclusive statement. However, we noticed that SEEDS and the decision-tree have much more to tell us about understanding and interpreting communities and we have barely scratched the surface.}
...
%The following information couldnÕt be added to Exchange:
...
The SEEDS \emph{snapshot} forms described in Section~\ref{fw} can also be explained in terms of the Stacey matrix \cite{sta02aa}. The matrix, shown in Figure~\ref{fig:stacey}, was introduced by Ralph Stacey to help make sense of management approaches using the two axes of ``agreement'' and ``certainty''.
Stacey notes that wherever there are high levels of certainty and agreement, decisions are \emph{simple}. However if either the certainty or agreement level drops, the situation becomes \emph{complicated}. If certainty is high but agreement is not, the situation is political; if the agreement is high but certainty is not, the situation is judgmental.

\begin{figure}[h!]
%\begin{wrapfigure}{r}{0.30\textwidth}
  \begin{center}
    \includegraphics[width=2.7in]{Stacey.pdf}    %
  \end{center}
\vspace{-5mm}
...
At the other extreme, in situations where the levels of certainty and agreement are very low, it may not be possible to manage and organize at all. This is the domain of \emph{anarchy} and chaos. Between \emph{complicated} and \emph{anarchy} lies a large \emph{complex} area.

The four SEEDS \emph{snapshot} forms are similar although not entirely identical to the simple, complicated, complex and anarchic (chaotic) domains. In the \emph{simple} form, the similarities are trivial to establish. The SEEDS \emph{complicated} form represents a slight lessening of certainty and/or agreement, but does not distinguish between Stacey's two versions of complicatedness: political and judgmental complications. The SEEDS \emph{complex} form is in agreement with the Stacey matrix, implying substantially lower certainty and/or agreement.

The SEEDS model lacks an anarchic form, which is not surprising as anarchy and chaos are antithetic to organisation. Nevertheless, we noticed that SEEDS can produce \emph{snapshots} consisting of mostly (or only) pivot-points. This case clearly reflects a situation where different units have radically different understandings of the organisation, and relates to the notion of ``organisational chaos'' \cite{sta02aa}.

Using SEEDS to understand these many dimensions is critical to elaborate organisation efficiency metrics, e.g.~to integrate models such as CMMI with ``community readiness'' indicators.
...
While we were able to carry out a complete case-study to ``reverse engineer'' SEEDS, this also taught us that the resulting method still has many limitations. 

First, SEEDS is currently based on qualitative evidence alone. To give statistical validity to the approach, quantitative metrics should be evaluated and developed. In addition, the SEEDS analysis framework is still very preliminary and based on organisational research, rather than focused to the benefits of development communities. Testing SEEDS on working development communities gave us many insights for improvement, however much work still needs to be done. 

Moreover, the validation of our method through a large industrial case study, validates snapshots-based evaluation of a community, but says little about validating its evolution. Last but not least, the method presented in Section \ref{method} was obtained as a refinement of guidelines presented in \cite{specissue} but is the result of exploratory research during the case study. Before the final version was obtained, many intermediate versions were worked-out in our case study, until desired \emph{snapshots} were obtained. This series of intermediate versions compromises the external validity of our case-study. More study is therefore needed to confirm the external validity of our approach. In summary, many critical research questions still remain open concerning SEEDS and its application. For example, what is the period of time to which a snapshot should be associated? How should the SEEDS feedback loop for tuning be instrumented? Even with these limitations, our results are encouraging.
...
%
In this paper we introduced a method to understand software development communities by taking ``snapshots'' of their state using observable attributes. The method uses previous work and emerges from applying previous validated results on a large industrial case-study. Through many iterations we refined the method into the final version of SEEDS, a method for ``Snapshot-based Evaluation and Evolution of Development communitieS''. This paper reports the final version of the method, and the case-study we applied it on for validation.
...
Elaborating SEEDS we concluded that the method produces an ``actionable'' state for the observed community, i.e.~a picture of the community that eases acting upon community characteristics. Applying the method in practice, we also recognised its many limitations and some threats to its validity. 
...
The authors would like to thank Mr. Ossi Korhonen (M.Sc.) and Ms. Minna R\"{a}is\"{a}nen (M.Sc.) for providing empirical evidence in the form of well-defined reports, for us to elaborate the results in this paper. We acknowledge the contribution of all employees in Company X that offered their time to confirm the results contained in this paper. 
